...
console.log(personAccount.addExpense(1));
...
function signUp() {
  ...
  let newUser = { username, email, password, isLoggedIn, createdAt };
...
// postavi funckiju
// napisati prompt za username i password
// pronađi find metodom - ako je username jednak users.username i ako je password jednak users.password promijeni false u true
function signIn() {
...
function rateProduct() {
  let product = prompt("Choose product: mobile phone, laptop or TV");
  let yourRating = prompt("Give your rating");

  let findProduct = products.find(
    (findProduct) => findProduct.name === product
  );
  console.log(typeof findProduct);
  if (findProduct) {
    findProduct.ratings.push({ userId: "BLA", rate: yourRating });
    // nisam promijenio rating
    console.log("valja");
...
//1. napisati funkcija koja izračunava avarage
//2. for petlja koja prikazuje rezultat
